...
**2026-08-03 cleanup note:** Track A (Large/Anchor) and Track B (Small/Young, condensed 7-phase) — the two
context-window-limited-model tracks this file originally offered alongside Track C — have been removed from
this file. They're superseded: **Track C (DeepSeek Methodology) below is the only prompt set now used**, for
projects of any size, since DeepSeek's 1M-token context window removes the reason Track A/B's
phase-count/Context-Pack tradeoffs existed in the first place (see "Why DeepSeek..." under Track C). Track
A's real output (LayerZero's dossier) still exists in the dataset and historical references to it elsewhere
in this file are left as-is — only the now-unused *prompt text* was removed, not the record of what
produced already-ingested data. Skip straight to "Track C" below for the prompts actually in use.

1. **Focus on one project at a time through the whole pipeline before starting another** — see
...
2. Run each phase **in dependency order, in one continuous chat** — Track C's whole point is that DeepSeek's
   1M-token context window removes the need for the Context Pack discipline Track A/B required (attaching a
   trimmed index of prior phases instead of full text); just paste each phase's prompt right after the last
   one in the same conversation and let the transcript itself be the context. See "How this changes the
   workflow vs. Track A's Context Pack" under Track C below for the full rationale.
3. If the project already has a `examples/Sentiment/<Project>.md` companion (Grok/X), paste it as additional
...
4. Export each phase's raw output as its own `.docx`, named `NN-<phasekey>.docx` with the **exact** phase
   ...
   still works for in-flight projects but should not be used for new ones.) A plain UTF-8 text file saved
   with a `.docx` extension works fine too — `tools/extract.py`'s `extract_docx()` falls back to reading it
   as-is when it isn't a real OOXML container (this is how the real Arbitrum run's files are actually saved;
   see `reset/README.md` for the automated pipeline that produces them this way).
5. Every prompt below shares the same closing/format rules — defined once, not repeated per phase. Every
