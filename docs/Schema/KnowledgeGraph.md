...
Specify how knowledge about **the knowledge-graph model** must be captured, structured, and validated within CIF's Schema layer (cross-cutting) (Cross-cutting — spans all stages).
...
Within the pipeline (Research → Knowledge → Ontology → Patterns → Reasoning → Framework → Applications), the knowledge-graph model is a distinct unit that must be captured explicitly so downstream stages can reason over it consistently and traceably.
