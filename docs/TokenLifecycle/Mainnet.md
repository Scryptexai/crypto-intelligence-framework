...
Specify how knowledge about **the mainnet phase** must be captured, structured, and validated within CIF's Token lifecycle layer (Patterns (Stage 4) — lifecycle).
...
Within the pipeline (Research → Knowledge → Ontology → Patterns → Reasoning → Framework → Applications), the mainnet phase is a distinct unit that must be captured explicitly so downstream stages can reason over it consistently and traceably.
